...
操作UIは「背景画像」「文字」の2つの折りたたみパネル（`<details>`）へ分ける。横に広い画面ではプレビューと背景画像パネルを左列、文字パネルと書き出しを右列へ置く。860px以下では同じ順序で1列へ縦積みし、スマートフォンでも各パネルを閉じて操作できるようにする。1600px以上ではコンテンツの左右へ 160×600 の広告レールをグリッドの実カラムとして追加する。レールは操作UIと重ならないよう `position: fixed` を使わず、追従は `position: sticky` で行う。
...
3. それ以外では、生成PNGをクリップボードへコピーし、手順を示すモーダルを開く。
4. Xへの遷移はモーダル内の「Xの投稿画面を開く」を押したときだけ行う。
5. いずれの経路でもハッシュタグとツールURLを本文へ含める。
6. `AbortError`（ユーザーのキャンセル）はエラーとして扱わない。
...
コピーの完了をユーザーが認識しないまま遷移すると貼り付けに至らないため、
`<dialog>` の `showModal()` で背景を暗くし、手順を読ませてから遷移させる。
コピーに失敗した場合は、モーダル内の文言を手動添付の案内へ切り替える。
